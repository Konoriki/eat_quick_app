...
import { getMenuItemsByCategory, getAllMenuItems } from '@/lib/markdown';
...
export default async function CategoryPage(props: { params: Params }) {
  ...
  return (
    <div className="max-w-7xl mx-auto py-12 px-4">
      <h1 className="text-5xl font-bold mb-8 text-center text-orange-600 capitalize">
      ...
      {items.length === 0 ? (
        <p className="text-center text-gray-600 text-lg">No items found in this category.</p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {items.map((item) => (
            <div
              key={item.slug}
              className="group bg-white rounded-[2rem] shadow-sm hover:shadow-2xl hover:-translate-y-2 overflow-hidden border border-slate-100 transition-all duration-500 flex flex-col"
            >
